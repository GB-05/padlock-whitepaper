...
key, 8 byte input hash, 8 byte output hash). The other optimization is to use
indexes to public keys and input hashes. There will be an array of all public
keys used. Rather than using the entire public key in an entry, you can instead
use an index to this array. The same goes for inputs. Each public key will have
an associated array of input hashes, listed in chronological order. In the
entry, you can use a 32 bit integer to refer to them, taking only 4 bytes
instead of 8.
