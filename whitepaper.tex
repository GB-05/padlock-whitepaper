...
\begin{abstract}
    ...
    creation of a scalable and private cryptocurrency. 
...
hash, is an input hash, an output hash, and a proof of work. These four byte arrays together,
...
for creating a scalable, private, non double-spendable cryptocurrency.
...
A coin can be divided into multiple fractions of a coin, specifying more than one
public key as the owner, and creating a new  coin in the process. Each coin is then representing a fraction of the value of the
original coin. When a transaction occurs, the sender can send a
fraction of the coin to the receiver, and the rest back to themselves, much like change in traditional currencies.
...
gigabytes. It grows around 200 megabytes per day when at max
...
transactions wanting to confirm than can fit in to the next block, or even the
...
infeasible for the vast majority of people, and will only be possible through a select few.
...
and signature individually hashed, and then used as an entry on the PADLOCK blockchain.
...
the coin had already been spent, and the transaction would not go through.
...
One way to attack this protocol would go as follows:
...
the barrier to entry for mining, making it inaccessible for a larger range of people, and
centralizing mining.
...
with \Gls{ASIC}, making algorithms specifically designed for \Gls{asic} ineffective.
...
persons transactions would be publicly known, making cryptocurrencies obsolete.
...
hard to piece together all of the transactions. This
