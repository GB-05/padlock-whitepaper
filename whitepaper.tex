...
\begin{abstract}
    ...
    of the input and the output, and only a hash of the signature. These are the
    only details that need to be publicly disclosed on the blockchain in order
    for a transaction recipient to be able to verify that the coins they have
    received were not double spent. This results in an incredibly small
    transaction size of only 48 bytes, allowing for around 4x more throughput
    than bitcoin.
\end{abstract}
...
algorithms.  However, those algorithms could be being used by other
...
with \Gls{ASIC} designed for those algorithms.
...
This nonce can be up to 8 bytes, and is then sent with the entry to the mempool.
Nodes have a minimum amount of work they will accept when considering whether to
add an entry to the mempool. The work requirement can change based on the size
of the mempool. If there are too many transactions in the mempool, nodes will up
the amount of work required to put an entry in the mempool.
